...
(function () {
    ...
    function listVideoHrefList() {
        const $videoSectionList = Array.from(document.querySelectorAll('.content')).filter(el => el.innerHTML.includes('http://guangzhou.ouchn.cn/theme/blueonionre/pix/he.png') &&
            !el.className.includes('act') &&
            !el.querySelector('.flexsections'));
        const watchedList = JSON.parse(localStorage.getItem('watchedList') || '[]');
        const videoLinkList = $videoSectionList
            .flatMap(el => el.querySelectorAll('.activityinstance > a'))
            .filter($a => watchedList.includes($a.href))
            .map($a => $a.href);
        return videoLinkList;
    }
    async function forTriggerVideoLink(hrefList) {
        for (let url of hrefList) {
            window.open(url, '_blank');
            localStorage.setItem('status', 'started');
            await wait(() => localStorage.getItem('status') === 'stopped');
        }
    }
